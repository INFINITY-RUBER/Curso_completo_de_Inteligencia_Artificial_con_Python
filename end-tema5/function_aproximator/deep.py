...
class DeepActor(torch.nn.Module):
    ...
    def __init__(self, input_shape, output_shape, device = torch.device("cpu")):
        ...
        para cada uno de los valores de output_shape. usando el algoritmo de CNN(red de convolucion neuronal)
        ...
        self.layer3 = torch.nn.Sequential(torch.nn.Conv2d(64, 64, 3, stride = 1, padding = 0),
                                          torch.nn.ReLU())
        self.layer4 = torch.nn.Sequential(torch.nn.Linear(64 * 7 * 7, 512), # convolucion lineal
                                          torch.nn.ReLU())
