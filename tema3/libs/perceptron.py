...
class SLP(torch.nn.Module):
    ...
    def __init__(self, input_shape, output_shape, device = torch.device("cpu")):
        ...
        """
        super(SLP, self).__init__()# llamara a la super clase y inicialisar los parametros de la misma
        self.device = device
        self.input_shape = input_shape[0]
        self.hidden_shape = 40
        self.linear1 = torch.nn.Linear(self.input_shape, self.hidden_shape)
        #linear1 lo que hace es reprecentar desde los datos de entreada a la capa oculta de 40 unidades (hidden_shape)
    ...
    def forward(self, x):
        ...
        # RELU se va activar si detcta un valor positvo
        x = self.out(x)
        return x